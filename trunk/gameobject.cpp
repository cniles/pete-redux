...
GameObject::GameObject(float x, float y, GameState* gamestate, btCollisionShape* collision_shape) {
  this->gamestate = gamestate;
  position = btVector3(x, y, 0);
  direction = -1;

  btVector3 initial_position = btVector3(x+0.5f, y+0.5f, 0);
  btMotionState* motion_state = 
    new GameObjectMotionState(btTransform(btQuaternion(0,0,0,1),initial_position), this);
  btScalar mass = 1.0;
